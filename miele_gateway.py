...
# Note: The @app.route decorators MUST come AFTER `app = Flask(__name__)`
...
def main_route(resource, explore_mode=False):
    ...
    # --- Path Handling (Trailing Slash Fix - kept from v2/v3) ---
    ...
    try:
        ...
        # --- Use ASCII encoding for the signing string (from v3) ---
        try:
            signing_bytes = signing_string.encode('ascii')
            if debug_log: print("Using ASCII encoding for signing string.")
        ...
        headers = {
            'Accept': accept_header, 'Date': current_time_gmt,
            'User-Agent': 'Miele@mobile 2.3.3 Android', 'Host': host,
            'Authorization': auth_header_value, 'Accept-Encoding': 'gzip',
        }
        ...
        if is_explore_request:
            try:
                ...
                # Basic HTML formatting for explore mode
                html_content = f"""
                <!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Explore: {host}{resource_path_on_device}</title>
                <style>body{{font-family:sans-serif;margin:1em;}} h1{{border-bottom:1px solid #ccc;}} pre{{white-space:pre-wrap;background:#f0f0f0;padding:1em;border:1px solid #ddd;}} a{{color:blue;}}</style></head>
                <body><h1>{host}{resource_path_on_device}</h1><pre>{json.dumps(json_data, indent=2)}</pre></body></html>"""
                return Response(html_content, mimetype='text/html')
            except json.JSONDecodeError as e:
                print(f"Warning: Decrypted data for explore mode is not valid JSON: {e}")
                html_content = f"""
                 <!DOCTYPE html><html lang="en"><head><title>Explore Error: Not JSON</title><style>.error{{color:red;}} body{{font-family:monospace;white-space:pre;}}</style></head>
                 <body><h1>Error: Response was not valid JSON</h1><p class="error">Path: {host}{resource_path_on_device}</p><hr><p>{decrypted_str}</p></body></html>"""
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=port, debug=False, use_reloader=True) # use_reloader=True helpful for dev
